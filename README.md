...
## Building
Note: for now I've been using WSL for development so I've also been building a Windows .dll so that I can load it on the Windows simulator on the host. You'll need to adjust `build.zig` to change the `std.zig.CrossTarget` of `simulator_lib` if your setup is different.
